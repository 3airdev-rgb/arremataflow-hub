...
export function ImageManagementSection({
  ...
  const persistOrder = async (next: ProjetoFoto[]) => {
    ...
    const response = await fetch("/api/project-images/order", {
      method: "POST",
      credentials: "same-origin",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ projectId: projetoId, ids: next.map((image) => image.id) }),
    });
    if (!response.ok)
      toast.error((await response.text()) || "Não foi possível salvar a ordem das fotos.");
  };
  ...
  const deleteImage = async (id: string) => {
    ...
    try {
      if (projetoId && !imgToDelete.file) {
        const response = await fetch(`/api/project-images/${imgToDelete.id}`, {
          method: "DELETE",
          credentials: "same-origin",
        });
        if (!response.ok)
          throw new Error((await response.text()) || "Não foi possível excluir a imagem.");
      ...
      {images.length === 0 ? (
        ...
        >
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Clique ou arraste imagens aqui para iniciar o envio.
          </p>
        </div>
      ) : (
            ...
            {images.length > 1 && (
              <>
                <Button
                  ...
                  className="absolute left-4 top-1/2 -translate-y-1/2 touch-visible opacity-100 sm:opacity-0 sm:group-hover:opacity-100 group-focus-within:opacity-100 focus-visible:opacity-100 transition-opacity rounded-full shadow-lg"
                  onClick={() =>
                ...
                <Button
                  ...
                  className="absolute right-4 top-1/2 -translate-y-1/2 touch-visible opacity-100 sm:opacity-0 sm:group-hover:opacity-100 group-focus-within:opacity-100 focus-visible:opacity-100 transition-opacity rounded-full shadow-lg"
                  onClick={() =>
...
function SortableItem({ image, isSelected, onClick, onDelete, onSetMain }: SortableItemProps) {
  ...
  return (
    <div
      ...
      onClick={onClick}
    >
      <AuthenticatedImage src={image.url} alt="" className="size-full object-cover" />

      {/* Drag Handle Overlay (Visible on Hover) */}
      <div
        {...attributes}
        {...listeners}
        className="absolute inset-0 bg-black/20 touch-visible opacity-100 sm:opacity-0 sm:group-hover:opacity-100 group-focus-within:opacity-100 focus-visible:opacity-100 transition-opacity flex items-center justify-center cursor-grab active:cursor-grabbing"
      >
      ...
      {/* Actions Dropdown */}
      <div className="absolute top-1 right-1 touch-visible opacity-100 sm:opacity-0 sm:group-hover:opacity-100 group-focus-within:opacity-100 focus-visible:opacity-100 transition-opacity">
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button
              ...
              className="size-9 sm:size-7 rounded-full shadow-sm"
            >
