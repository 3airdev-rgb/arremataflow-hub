...
const CurrencyInput = React.forwardRef<HTMLInputElement, CurrencyInputProps>(
  (
    { value, onValueChange, wholeReais = false, maxValue, className, onFocus, onBlur, ...props },
    ref,
  ) => {
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (wholeReais) {
        ...
        const normalized =
          decimalParts.length > 0 ? `${integerPart || "0"},${decimalPart}` : integerPart;
        const parsedValue = normalized ? Number(normalized.replace(",", ".")) : 0;
        const limitedValue = Number.isFinite(parsedValue)
          ? Math.min(parsedValue, maxValue ?? Number.MAX_SAFE_INTEGER)
          : 0;

        setDisplayValue(
          limitedValue === parsedValue ? `R$ ${normalized}` : formatCurrency(limitedValue),
        );
