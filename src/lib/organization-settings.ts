...
export const updateOrganizationSettings = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    await db.transaction(async (tx) => {
      await tx.update(schema.organizations).set({ ...data, updatedAt: new Date() }).where(eq(schema.organizations.id, membership.organizationId));
      await tx.insert(schema.auditLogs).values({ organizationId: membership.organizationId, actorId: userId, action: "organization.updated", entityType: "organization", entityId: membership.organizationId, metadata: { fields: Object.keys(data) } });
    });
