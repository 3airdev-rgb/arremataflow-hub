...
function issueMessage(issue: unknown) {
  ...
  return label && !rawMessage.toLowerCase().includes(label.toLowerCase())
    ? `${label}: ${rawMessage}`
    : rawMessage;
}

export function readableValidationMessage(
  error: unknown,
  fallback = "Revise os campos informados.",
) {
  ...
  try {
    ...
    if (Array.isArray(parsed)) {
      const messages = [
        ...new Set(parsed.map(issueMessage).filter((item): item is string => Boolean(item))),
      ];
...
export function showValidationAlert(error: unknown, fallback?: string) {
  ...
  window.dispatchEvent(
    new CustomEvent(VALIDATION_ALERT_EVENT, {
