...
const movementInput = z.object({
  ...
  amount: z.number().finite().positive().max(999_999_999_999_999.99),
...
async function financialContext(projectId: string, requireManager = false) {
  ...
  if (requireManager && !["owner", "admin"].includes(membership.role)) throw new Error("Sem permissão para registrar movimentações.");
...
const mapMovement = (row: any) => ({
  ...
  valor: Number(row.amount), status: row.status, tipo: row.type,
  document_holder_name: row.holderName, document_holder_document: row.holderDocument,
  document_holder_type: row.holderType, document_type: row.documentType,
  comprovanteUrl: null, comprovanteUrls: [] as string[],
});
...
export const listFinancialMovements = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    const rows = await db.select().from(schema.financialMovements).where(and(
    ...
    )).orderBy(desc(schema.financialMovements.movementDate), desc(schema.financialMovements.createdAt));
    const ids = rows.map((row) => row.id);
    const receipts = ids.length ? await db.select({ id: schema.documents.id, movementId: schema.documents.financialMovementId })
    ...
    return rows.map((row) => ({
      ...mapMovement(row),
      comprovanteUrls: receipts.filter((item) => item.movementId === row.id).map((item) => `/api/documents/${item.id}`),
    }));
  });
...
export const createFinancialMovement = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    return db.transaction(async (tx) => {
      const [created] = await tx.insert(schema.financialMovements).values({
        organizationId: membership.organizationId, projectId: data.projectId,
        type: data.type, description: data.description, category: data.category,
        amount: data.amount.toFixed(2), holderName: data.holderName || null,
        holderDocument: document, holderType: data.type === "receita" ? "Origem" : "Destinatário",
      ...
      await tx.insert(schema.auditLogs).values({
        organizationId: membership.organizationId, projectId: data.projectId,
        actorId: session.user.id, action: "financial_movement.created",
        entityType: "financial_movement", entityId: created.id,
...
export const deleteFinancialMovement = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const { db, schema, session, membership } = await financialContext(data.projectId, true);
    return db.transaction(async (tx) => {
      const [deleted] = await tx.delete(schema.financialMovements).where(and(eq(schema.financialMovements.id, data.id), eq(schema.financialMovements.projectId, data.projectId), eq(schema.financialMovements.organizationId, membership.organizationId))).returning();
      if (!deleted) throw new Error("Movimentação não encontrada.");
      await tx.insert(schema.auditLogs).values({ organizationId: membership.organizationId, projectId: data.projectId, actorId: session.user.id, action: "financial_movement.deleted", entityType: "financial_movement", entityId: deleted.id, metadata: { type: deleted.type, category: deleted.category, amount: deleted.amount } });
      return { ok: true };
    });
  });
...
export const findFinancialHolder = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    const [contact] = await db.select({ name: schema.contacts.name }).from(schema.contacts).where(and(
      eq(schema.contacts.organizationId, membership.organizationId), eq(schema.contacts.document, document),
    )).limit(1);
    if (contact) return contact.name;
    const [movement] = await db.select({ name: schema.financialMovements.holderName }).from(schema.financialMovements).where(and(
      eq(schema.financialMovements.organizationId, membership.organizationId),
      eq(schema.financialMovements.holderDocument, document),
    )).orderBy(desc(schema.financialMovements.createdAt)).limit(1);
