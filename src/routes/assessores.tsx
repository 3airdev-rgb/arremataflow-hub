...
export const Route = createFileRoute("/assessores")({
  head: () => ({
    meta: [
      ...
      { property: "og:description", content: "Projetos e resultados sob responsabilidade do assessor." },
    ],
  }),
    ...
    <AppLayout title="Painel do Assessor" subtitle={`${projetosDoAssessor.length} projeto(s) disponível(is)`}>
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        ...
        <Select value={statusFilter} onValueChange={setStatusFilter}>
          <SelectTrigger className="w-full sm:w-56" aria-label="Filtrar projetos por status"><SelectValue /></SelectTrigger>
          <SelectContent>
      ...
      <div className="grid gap-5 md:grid-cols-2">
        {projetosVisiveis.map((project) => (
          <Link key={project.id} to="/projetos/$id" params={{ id: project.id }} className="surface-card block overflow-hidden transition-shadow hover:shadow-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand" aria-label={`Visualizar projeto ${project.nome}`}>
            <img src={project.foto} alt={`Foto do imóvel ${project.nome}`} className="h-48 w-full object-cover" />
            <div className="p-5">
              <h2 className="text-xl">{project.nome}</h2>
              <p className="text-sm text-muted-foreground">{project.endereco} — {project.cidade}</p>
              <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
                <StatusBadge status={project.status} />
                <p className="text-xs text-muted-foreground">Etapa atual: <span className="font-medium text-foreground">{project.etapa}</span></p>
              </div>
              <div className="mt-4">
                <div className="mb-1 flex justify-between text-xs text-muted-foreground"><span>Evolução do projeto</span><span>{project.progresso}%</span></div>
                <div className="relative h-2.5 overflow-hidden rounded-full bg-muted" role="progressbar" aria-valuenow={project.progresso} aria-valuemin={0} aria-valuemax={100}>
                  <div className="absolute inset-0 rounded-full transition-[clip-path]" style={{ background: "linear-gradient(90deg, #ef1b1b 0%, #ff6814 30%, #ffc400 58%, #08b85a 100%)", clipPath: `inset(0 ${100 - Math.min(Math.max(project.progresso, 0), 100)}% 0 0)` }} />
                </div>
              </div>
