...
O endereço padrão para acessar e validar o aplicativo localmente é **http://127.0.0.1:8081**. Ele é servido pelo serviço `app` do Docker Compose definido em `compose.yaml`. Depois de alterar a interface, reconstrua esse serviço com `docker compose --env-file .env.docker up -d --build app` e confira a mudança nesse endereço antes de entregá-la.

`pnpm dev` inicia uma instância de desenvolvimento separada em `localhost:3000`. Ela serve para testes rápidos durante a implementação, mas não substitui a validação no endereço padrão usado pelos usuários.

Para desenvolver localmente, utilize Node.js, pnpm, Docker Desktop e o PostgreSQL definido em `compose.yaml`.
...
Antes de enviar alterações, rode `pnpm lint`, `pnpm typecheck` e `pnpm test:ci`. O pipeline de qualidade executa as mesmas verificações.

Segurança, LGPD, backup e implantação estão em [SECURITY.md](SECURITY.md).
...
Acessíveis sem login, por token individual ou fluxo de autenticação:

- `/redefinir-senha`: recuperação de senha.
- `/vistoria/:token`: formulário de vistoria preenchido pelo vistoriador convidado.
- `/confirmar-participacao/:token`: resposta de convidados a reuniões de tarefas.

## Design System — ArremataFlow

### Conceito visual

O ArremataFlow deve transmitir confiança, organização, segurança jurídica e controle financeiro. O design segue uma estética SaaS moderna, limpa e corporativa, priorizando produtividade e leitura rápida das informações.

### Paleta de cores

| Grupo       | Nome              | Valor     |
| ----------- | ----------------- | --------- |
| Primária    | Azul Petróleo     | `#0F3D56` |
| Primária    | Azul Médio        | `#1E5F8B` |
| Primária    | Azul Claro        | `#DCEFF8` |
| Secundária  | Verde Sucesso     | `#16A34A` |
| Secundária  | Verde Claro       | `#DCFCE7` |
| Neutra      | Branco            | `#FFFFFF` |
| Neutra      | Fundo             | `#F8FAFC` |
| Neutra      | Cinza Claro       | `#E5E7EB` |
| Neutra      | Cinza Médio       | `#6B7280` |
| Neutra      | Texto Principal   | `#1F2937` |
| Alerta      | Amarelo           | `#F59E0B` |
| Alerta      | Vermelho          | `#DC2626` |
| Alerta      | Azul Informação   | `#2563EB` |
...
Sidebar esquerda, com ícones e texto:

Dashboard, Projetos, Imóveis, Investidores, Assessores, Tarefas, Documentos, Regularização, Posse, Obras, Financeiro, Resultados, Notificações, Configurações e Perfil.

### Dashboard

Topo com pesquisa global, notificações, empresa ativa e perfil do usuário.

KPIs em cards: Projetos Ativos, Regularizações, Pendências, Em Posse, Em Reforma, À Venda, Capital Investido, Honorários e Resultado Projetado.

Abaixo dos KPIs: pipeline Kanban, calendário de vencimentos, tarefas do dia, alertas críticos e últimas movimentações.

### Página do projeto

Banner superior com foto principal do imóvel, código, endereço, modalidade, status, investidores e assessores.

Abas: Visão Geral, Financeiro, Regularização, Posse, Documentos, Tarefas, Reforma, Venda, Resultado e Histórico.
...
Cards de KPI, timeline, Kanban, data grid com filtros, upload por drag-and-drop, visualizador de documentos, linha do tempo do projeto, calendário de tarefas, modal lateral (Drawer), dialogs de confirmação, badges coloridas, breadcrumbs, tabs e toasts de notificação.
