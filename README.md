...
O endereço padrão para acessar e validar o aplicativo localmente é
**http://127.0.0.1:8081**. Ele é servido pelo serviço `app` do Docker Compose
definido em `compose.yaml`. Depois de alterar a interface, reconstrua esse
serviço com `docker compose --env-file .env.docker up -d --build app` e confira a
mudança nesse endereço antes de entregá-la.

`pnpm dev` inicia uma instância de desenvolvimento separada em `localhost:3000`.
Ela serve para testes rápidos durante a implementação, mas não substitui a
validação no endereço padrão usado pelos usuários.

Para desenvolver localmente, utilize Node.js, pnpm, Docker Desktop e o PostgreSQL
definido em `compose.yaml`.
