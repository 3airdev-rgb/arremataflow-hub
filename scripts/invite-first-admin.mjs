...
try {
  ...
  if (existingUsers.rows[0].count === 0) {
    ...
    await client.query(
      "insert into organization_members (organization_id, user_id, role, status) values ($1, $2, 'admin', 'invited')",
    ...
    if (
      ...
      throw new Error(
        "A reemissão foi recusada porque o convite não está pendente ou não corresponde ao proprietário original.",
      );
    }
    userId = current.id;
    invitedName = current.name;
    await client.query("delete from verifications where value = $1 and identifier like $2", [
      userId,
      "reset-password:%",
    ]);
  }
...
console.log(
  delivery.mode === "email"
    ...
    : `Convite local salvo em: ${delivery.path}`,
);
