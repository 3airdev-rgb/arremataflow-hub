...
const sql = await readFile(
  new URL("../drizzle/0013_active_organization.sql", import.meta.url),
  "utf8",
);
...
try {
  ...
  const exists = await client.query(
    `select 1 from information_schema.columns where table_schema='public' and table_name='users' and column_name='active_organization_id'`,
  );
